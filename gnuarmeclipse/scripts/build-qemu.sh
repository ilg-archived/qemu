...
if [ -d /media/Work ]
then
  QEMU_WORK_FOLDER=${QEMU_WORK_FOLDER:-"/media/Work/qemu"}
else
  QEMU_WORK_FOLDER=${QEMU_WORK_FOLDER:-${HOME}/Work/qemu}
fi
...
QEMU_BUILD_FOLDER="${QEMU_WORK_FOLDER}/build/${QEMU_TARGET}"
QEMU_INSTALL_FOLDER="${QEMU_WORK_FOLDER}/install/${QEMU_TARGET}"
...
then
  ...
  cd "${QEMU_GIT_FOLDER}"
  git checkout gnuarmeclipse-cortexm
...
then
  ...
  --extra-cflags=-DGNU_ARM_ECLIPSE=1 \
...
/usr/bin/install -c -m 644 "${QEMU_GIT_FOLDER}/gnuarmeclipse/info/INFO-osx.txt" \
  "${QEMU_INSTALL_FOLDER}/qemu/INFO.txt"
mkdir -p "${QEMU_INSTALL_FOLDER}/qemu/gnuarmeclipse"
/usr/bin/install -c -m 644 "${QEMU_GIT_FOLDER}/gnuarmeclipse/info/BUILD-osx.txt" \
...
QEMU_INSTALLER=${QEMU_OUTPUT}/gnuarmeclipse-qemu-\
${QEMU_TARGET}-${OUTFILE_VERSION}.pkg
...
then
  ...
  echo "File ${QEMU_INSTALLER} created."
else
  echo "Buld failed."
fi
