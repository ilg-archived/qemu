...
static void stm32f411xx_iwdg_create_objects(Object *obj, JSON_Object *svd, const char *name)
{
    ...
    // Registers. 
    state->u.f4.reg.kr = cm_object_get_child_by_name(obj, "KR");
    state->u.f4.reg.pr = cm_object_get_child_by_name(obj, "PR");
    state->u.f4.reg.rlr = cm_object_get_child_by_name(obj, "RLR");
    state->u.f4.reg.sr = cm_object_get_child_by_name(obj, "SR");
    
    
    // KR bitfields.
    state->u.f4.fld.kr.key = cm_object_get_child_by_name(state->u.f4.reg.kr, "KEY");  
    
    // PR bitfields.
    state->u.f4.fld.pr.pr = cm_object_get_child_by_name(state->u.f4.reg.pr, "PR");  
    
    // RLR bitfields.
    state->u.f4.fld.rlr.rl = cm_object_get_child_by_name(state->u.f4.reg.rlr, "RL");  
    
    // SR bitfields.
    state->u.f4.fld.sr.pvu = cm_object_get_child_by_name(state->u.f4.reg.sr, "PVU"); 
    state->u.f4.fld.sr.rvu = cm_object_get_child_by_name(state->u.f4.reg.sr, "RVU");  
}
...
static void stm32_iwdg_realize_callback(DeviceState *dev, Error **errp)
{
    ...
    case STM32_FAMILY_F4:
        ...
        if (capabilities->f4.is_411xx ) {
            ...
            stm32f411xx_iwdg_create_objects(obj, cm_state->svd_json, periph_name);
            ...
            // TODO: add callbacks.
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_iwdg_xxx_pre_read_callback);
            // peripheral_register_set_post_read(state->f4.reg.xxx, &stm32_iwdg_xxx_post_read_callback);
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_iwdg_xxx_pret_read_callback);
            // peripheral_register_set_post_write(state->f4.reg.xxx, &stm32_iwdg_xxx_post_write_callback);
