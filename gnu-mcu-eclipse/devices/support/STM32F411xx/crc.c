...
static void stm32f411xx_crc_create_objects(Object *obj, JSON_Object *svd, const char *name)
{
    ...
    // IDR bitfields.
    state->u.f4.fld.idr.idr = cm_object_get_child_by_name(state->u.f4.reg.idr, "IDR");  
    
    // CR bitfields.
    state->u.f4.fld.cr.cr = cm_object_get_child_by_name(state->u.f4.reg.cr, "CR");  
}
...
static void stm32_crc_realize_callback(DeviceState *dev, Error **errp)
{
    ...
    case STM32_FAMILY_F4:
        ...
        if (capabilities->f4.is_411xx ) {
            ...
            stm32f411xx_crc_create_objects(obj, cm_state->svd_json, periph_name);
            ...
            // TODO: add callbacks.
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_crc_xxx_pre_read_callback);
            // peripheral_register_set_post_read(state->f4.reg.xxx, &stm32_crc_xxx_post_read_callback);
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_crc_xxx_pret_read_callback);
            // peripheral_register_set_post_write(state->f4.reg.xxx, &stm32_crc_xxx_post_write_callback);
