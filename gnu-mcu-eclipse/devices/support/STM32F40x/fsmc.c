...
static void stm32_fsmc_realize_callback(DeviceState *dev, Error **errp)
{
    ...
    case STM32_FAMILY_F4:
        ...
        if (capabilities->f4.is_40x ) {
            ...
            stm32f40x_fsmc_create_objects(obj, cm_state->svd_json, periph_name);
            ...
            // TODO: add callbacks.
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_fsmc_xxx_pre_read_callback);
            // peripheral_register_set_post_read(state->f4.reg.xxx, &stm32_fsmc_xxx_post_read_callback);
            // peripheral_register_set_pre_read(state->f4.reg.xxx, &stm32_fsmc_xxx_pret_read_callback);
            // peripheral_register_set_post_write(state->f4.reg.xxx, &stm32_fsmc_xxx_post_write_callback);
