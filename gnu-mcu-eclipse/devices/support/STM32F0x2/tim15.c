...
#include <hw/cortexm/stm32/tim15.h>
...
static void stm32f0x2_tim15_create_objects(Object *obj, JSON_Object *svd, const char *name)
{
...
#if 0
static peripheral_register_t stm32_tim15_xxx_pre_write_callback(Object *reg,
        Object *periph, uint32_t addr, uint32_t offset, unsigned size,
        peripheral_register_t value, peripheral_register_t full_value)
{
    STM32TIM15State *state = STM32_TIM15_STATE(periph);
...
static void stm32_tim15_xxx_post_write_callback(Object *reg, Object *periph,
...
static peripheral_register_t stm32_tim15_xxx_pre_read_callback(Object *reg,
        Object *periph, uint32_t addr, uint32_t offset, unsigned size)
{
    STM32TIM15State *state = STM32_TIM15_STATE(periph);
...
static void stm32_tim15_xxx_post_read_callback(Object *reg, Object *periph,
        uint32_t addr, uint32_t offset, unsigned size)
{
    STM32TIM15State *state = STM32_TIM15_STATE(periph);
...
static bool stm32_tim15_is_enabled(Object *obj)
{
    STM32TIM15State *state = STM32_TIM15_STATE(obj);
...
static void stm32_tim15_instance_init_callback(Object *obj)
{
...
static void stm32_tim15_realize_callback(DeviceState *dev, Error **errp)
{
    qemu_log_function_name();

    // Call parent realize().
    if (!cm_device_parent_realize(dev, errp, TYPE_STM32_TIM15)) {
        return;
    }
    ...
    STM32TIM15State *state = STM32_TIM15_STATE(dev);
    ...
    case STM32_FAMILY_F0:
        ...
        if (capabilities->f0.is_0x2) {
            ...
            stm32f0x2_tim15_create_objects(obj, cm_state->svd_json, periph_name);
            ...
            // TODO: add callbacks.
            // peripheral_register_set_pre_read(state->f0.reg.xxx, &stm32_tim15_xxx_pre_read_callback);
            // peripheral_register_set_post_read(state->f0.reg.xxx, &stm32_tim15_xxx_post_read_callback);
            // peripheral_register_set_pre_read(state->f0.reg.xxx, &stm32_tim15_xxx_pret_read_callback);
            // peripheral_register_set_post_write(state->f0.reg.xxx, &stm32_tim15_xxx_post_write_callback);

            // TODO: add interrupts.

// TODO: remove this if the peripheral is always enabled.
            snprintf(enabling_bit_name, sizeof(enabling_bit_name) - 1,
                DEVICE_PATH_STM32_RCC "/AHB1ENR/TIM15EN");
} else {
            assert(false);
        }
...
static void stm32_tim15_reset_callback(DeviceState *dev)
{
    qemu_log_function_name();

    // Call parent reset(); this will reset all children registers.
    cm_device_parent_reset(dev, TYPE_STM32_TIM15);
}

static void stm32_tim15_class_init_callback(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->reset = stm32_tim15_reset_callback;
    dc->realize = stm32_tim15_realize_callback;

    // TODO: remove this if the peripheral is always enabled.
    PeripheralClass *per_class = PERIPHERAL_CLASS(klass);
    per_class->is_enabled = stm32_tim15_is_enabled;
}

static const TypeInfo stm32_tim15_type_info = {
    .name = TYPE_STM32_TIM15,
    .parent = TYPE_STM32_TIM15_PARENT,
    .instance_init = stm32_tim15_instance_init_callback,
    .instance_size = sizeof(STM32TIM15State),
    .class_init = stm32_tim15_class_init_callback,
    .class_size = sizeof(STM32TIM15Class) };

static void stm32_tim15_register_types(void)
{
    type_register_static(&stm32_tim15_type_info);
}

type_init(stm32_tim15_register_types);
